...
public class Dijkstra {
	...
	int[] dist;											// keeps track of the distances of inputNode node to each of the other nodes in the graph. dist[i] is the value of the shortest distance from inputNode node to node i
	int[] prev;											// keeps track of the predecessor in the path from inputNode node to each of the other nodes in the graph
	boolean[] visited;									// keeps track of all nodes visited by the algorithm
	List<Integer> Q = new ArrayList<Integer>();			// Create a set of all the unvisited nodes
	...
	private void evaluateDistanceToNeighbors(int u) {
		for (Edge e: inputGraph.graph.get(new Node(u))) {						  // for every node v, adjacent to u, if there is an edge from u to v, where v is not visited and
	 ...
	 */
	List<Integer> getPreviousList(int[] prev, int dest) {
	 ...
	 */
	void printArrBoolean(boolean[] arr) {
		for (int i = 0; i < arr.length; i++)
			System.out.print(arr[i] + " ");
		System.out.println();
	}
